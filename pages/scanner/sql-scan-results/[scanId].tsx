  ...
  useEffect(() => {
    const fetchResults = async () => {
      ...
      try {
        ...
        const response = await axios.get(`${API_URL}/api/sql-result/${parsedScanId}`, {
        ...
        setMessage(
          data.length > 0
            ? 'SQL injection scan results loaded successfully.'
  ...
  return (
    <Layout>
      <main className="flex flex-col items-center justify-center min-h-screen py-10 bg-gray-900 text-gray-100">
        ...
        {loading && (
          <p className="text-gray-400 mb-5 text-lg animate-pulse">Loading SQL results...</p>
        )}
          ...
          <div className="w-full max-w-4xl space-y-6">
            {results.length === 0 ? (
              ...
              results.map((result, index) => (
                <div
                  key={result.vulnerabilityId ?? index}
                  className="bg-gray-800 p-6 rounded-lg shadow-lg hover:shadow-xl transition-shadow"
                >
                  <div className="flex items-center justify-between mb-4">
                    <h2 className="text-xl font-medium text-red-400">
                      Vulnerability: {result.vulnerabilityType ?? 'Unknown'}
                    </h2>
                    <span
                  ...
                  <p className="text-gray-300 mb-2">
                    <strong>Description:</strong> {result.details}
                  </p>
                    ...
                    <div className="mt-4 text-sm text-gray-300 bg-gray-700 p-4 rounded-md">
                      ...
                      <ul className="list-disc list-inside mt-2 space-y-1">
                        ...
                        <li>
                          ...
                          {truncateText(result.payloadUsed ?? '', 50)}
                        </li>
                        <li>
                          <strong>Summary:</strong>{' '}
                          {truncateText(result.summary, 400)}
                        </li>
                      </ul>
                    </div>
                  )}
