...
export default function SubdomainResult() {
  ...
  const handleDownload = async () => {
    ...
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/report/subzy`, {
      method: 'POST',
      headers: {
    ...
    const getTextColor = (line: string) => {
      if (/not vulnerable/i.test(line)) return 'text-green-600 font-semibold';      // ❌ Not vulnerable = red
      if (/vulnerable/i.test(line)) return 'text-red-600 font-semibold';        // ✅ Vulnerable = green
      if (/error|fail/i.test(line)) return 'text-yellow-500 font-semibold';       // ⚠️ Error/fail = yellow
      return 'text-white';
    };
    ...
    <Layout>
      <div className="p-6 max-w-3xl mx-auto">
        ...
        <ul className="list-disc pl-6 space-y-2">
          {results.map((line, i) => (
            <li key={i} className={getTextColor(line)}>
              {line}
            </li>
          ))}
        </ul>
